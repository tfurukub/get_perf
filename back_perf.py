...
class NtnxRestApi:
    ...
    def get_vm_perf(self,vm_uuid,start,end,interval):
        sub_url = "vms/"+vm_uuid+"/stats"+"/?metrics=hypervisor_cpu_usage_ppm%2Chypervisor_memory_usage_ppm&startTimeInUsecs="+start+"&endTimeInUsecs="+end+"&intervalInSecs="+interval
    ...
    try:
        ...
        for entity in vms.get("entities"):
            ...
            for entity2 in perf.get("statsSpecificResponses"):
                print("metrics: {}".format(entity2.get('metric')))
                for i in range(len(entity2.get('values'))):
                    t =  start/1000000 + i*30
                    print("{0} {1}".format(datetime(*time.localtime(t)[:6]),entity2.get('values')[i]))
